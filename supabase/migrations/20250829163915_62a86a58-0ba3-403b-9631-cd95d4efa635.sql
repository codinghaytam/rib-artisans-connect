...
-- Recreate views without explicit security mode (defaults to invoker)
CREATE VIEW public.artisan_public_profiles AS
SELECT 
  ap.id,
  ap.user_id,
  ap.category_id,
...
CREATE VIEW public.artisan_contact_profiles AS
SELECT 
  ap.id,
  ap.user_id,
  ap.category_id,
